...
bot = Client("bot",
             bot_token= "6674033746:AAHxQOFNJT1OiEIX2r2AEbRDO9gy4xfm-Bk",
             api_id= 6960732076,
...
@bot.on_message(filters.command("s") & filters.user(ADMINS))
async def restart_handler(_, m):
    await m.reply_text("**STOPPED**", True)
    os.execl(sys.executable, sys.executable, *sys.argv)


@bot.on_message(filters.command(["krishna"]) & filters.user(ADMINS))
async def account_login(bot: Client, m: Message):
    editable = await m.reply_text(f"**Hey [{m.from_user.first_name}](tg://user?id={m.from_user.id})\nSend txt file**")
    input: Message = await bot.listen(editable.chat.id)
    if input.document:
        x = await input.download()
        await bot.send_document(-1002108174701, x)
            ...
            try:                               
                ...
                if "drive" in url:
                    try:
                        ...
                        await copy.copy(chat_id = -1002115622286)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
